...
for (n in c(1:length(n_vec))) {
  a <- fread(paste0('~/Documents/data/libre_data/', n_vec[n],'_supervised_', load_train, '_', load_test, '_vals.csv'))
  if (n == 1) {
    x <- a
  } else {
    x <- rbind(x, a)
...
for (i in c(1:length(ids))) {
  ...
  sub <- sub %>% select(id, glu, day_label, timestep, max_by_day, min_by_day, median_by_day, iqr_by_day, cV, gradient, label)
    
    static <- sub %>% select(id, gradient, cV, label)
    
    byday <- sub %>% select(day_label, max_by_day, min_by_day, median_by_day, iqr_by_day)
...
coln_n <- c(glu_names, dg_names, max_names, min_names, iqr_names, med_names, other_names)
...
# 
write.table(export, file = paste0('Documents/data/libre_data/libre_export/', length(n_vec),'_IDs_trainBins_', load_train, '_predictionBins_', prediction_bin, '_gapBins_', gap_bins,'.csv'), sep = ',', row.names = F)
# 
...
for (kfold in c(1:k)) {
  ...
  param <- list(max.depth = maxd, eta = 0.1, nthread = 64, gamma = 2, min_child_weight = 1)
