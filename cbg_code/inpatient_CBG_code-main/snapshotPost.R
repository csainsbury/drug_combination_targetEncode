...
file_list <- list.files(path="~/projects/sanofi_inpatient/CBGdata_pre_and_post//")

for (i in seq(1, length(file_list), 1)) {
  print(i)
  name <- paste("~/projects/sanofi_inpatient/CBGdata_pre_and_post/", file_list[i], sep = "")
...
abline(v = returnUnixDateTime("01/03/2019 12:00AM") / (60*60*24))
    ...
    tempSetDT$roundedFractionalDay <- round(tempSetDT$fractionalDay, 3)
    ...
    hist(tempSetDT[BatteryVoltage > 3.38 & BatteryVoltage < 3.4]$fractionalDay)
    ...
    # findMatch <- function(glu_1_case, glu_2_pool, fractionalTime_1_case, fractionalTime_2_pool, priorTime_1_case, priorTime2_pool, timeWindow) {
      # glu_1_case = highV$GLUn[64]; glu_2_pool = lowerV$GLUn; fractionalTime_1_case = highV$roundedFractionalDay[64]; fractionalTime_2_pool = lowerV$roundedFractionalDay; priorTime_1_case = highV$meter_prior_measurement[64]; priorTime2_pool = lowerV$meter_prior_measurement; timeWindow = 10
      
      poolDT <- data.table(glu_2_pool, fractionalTime_2_pool, priorTime2_pool)
      matchPool <- poolDT[fractionalTime_1_case == fractionalTime_2_pool]
      output = matchPool[sample(nrow(matchPool), 1)]
      
      return(list(output$glu_2_pool, output$priorTime2_pool))
    ...
    for (tt in c(1:length(pVal_vector))) {
      ...
      highV[, c("matched_glu", "matched_prior") := findMatch(GLUn, lowerV$GLUn, roundedFractionalDay, lowerV$roundedFractionalDay, location_prior_measurement, lowerV$location_prior_measurement, 2), by=.(uniqueID)]
    ...
    for (jj in seq(1, ncol(matchedCBG_frame), 1)) {
      h = hist(matchedCBG_frame[, jj], plot = F, breaks = seq(1, 28, 1))
      end_col = ncol(matchedCBG_frame)
      if (jj == 1) {
        plot(h$counts, cex = 0, ylim = c(0, 900))
    ...
    for (w in c(1:nrow(wilcoxFrame))) {
      ...
      for (v in c(1:ncol(wilcoxFrame))) {
        ...
        wilcoxFrame_hypoValues[w, v] <- hypoTest$p.
      }
    }
    ...
    x = boxplot(dataDT$GLUn ~ cut(dataDT$BatteryVoltage, breaks = seq(3.3, 3.42, 0.02)), varwidth = T)
    x = boxplot(dataDT$Temperature ~ cut(dataDT$BatteryVoltage, breaks = seq(3.3, 3.42, 0.02)), varwidth = T)
